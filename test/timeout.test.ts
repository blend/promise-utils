...
test(
  'resolves appropriate value when function finishes first',
...
test(
  'handles argument passing appropriately',
  async (t: TestContext): Promise<void> => {
    const ret = await promiseUtils.timeout(
...
test(
  'throws errors when delays are too long',
  async (t: TestContext): Promise<void> => {
    const ret = await promiseUtils.invert(promiseUtils.timeout(
